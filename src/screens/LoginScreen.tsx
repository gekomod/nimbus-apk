...
export default function LoginScreen({ onBack, onSuccess, serverUrl }: Props) {
    ...
    >
      <View style={styles.root}>
        ...
        <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled">
          ...
          <Field label="Nazwa użytkownika" value={user} onChange={setUser} placeholder="admin" autoFocus />
          <Field
            label="Hasło"
            value={pass}
          ...
          <View style={styles.endpointBox}>
            <Text style={styles.endpointText}>
              ...
              {' '}{display}/api/login{'\n'}
              <Text style={{ color: C.textMute }}>{`{ username, password }`}</Text>
            </Text>
          </View>
        </ScrollView>
...
const styles = StyleSheet.create({
  ...
  errorBox: {
    ...
    borderWidth: 1,
    borderColor: 'rgba(241,77,76,0.4)',
  },
  errorText: {
    fontSize: 13,
    color: C.red,
